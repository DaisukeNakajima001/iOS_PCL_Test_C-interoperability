...
std::string PCLMesher::saveMeshAsOBJWrapper(const StdVectorSIMD3& pointCloud, const char* filePath) {
    std::string filePathStr(filePath); // const char* を std::string に変換
    return saveMeshAsOBJ(pointCloud.vector, filePathStr);
}

std::string PCLMesher::saveMeshAsOBJ(const std::vector<simd_float3>& pointCloud, const std::string& filePath) {
